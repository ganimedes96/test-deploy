...
import { Banknote, CheckCircle, ChevronLeft, ShoppingBag } from "lucide-react"
  ...
  return (
    <header className={"w-full flex text-gray-400  mt-[94px]  items-center justify-center "}>
      ...
      <div className={`w-4/12 border-[1px] ${activeLink === 'CART' ? 'border-orange-300 bg-orange-50 text-orange-500' : 'border-gray-300'}  py-2 flex flex-col items-center justify-center`}>
        <ShoppingBag size={28} strokeWidth={1} className={`${activeLink === 'CART' ? 'text-orange-500' : 'text-gray-400'}}`} />
        Carrinho
      </div>
      ...
      <div className={`w-4/12 border-[1px] ${activeLink === 'PAYMENT' ? 'border-orange-300 bg-orange-50 text-orange-500' : 'border-gray-300'}  py-2 flex flex-col items-center justify-center`}>
        <Banknote size={28} strokeWidth={1} className={`${activeLink === 'CART' ? 'text-orange-500' : 'text-gray-400'}}`} />
        Pagamento
      </div>
